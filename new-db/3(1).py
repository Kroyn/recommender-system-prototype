...
def load_data():
    ...
    try:
        ...
        # Зміна тут: використовуємо 'date' замість 'timestamp'
        movies_df = pd.read_csv(movies_path, usecols=['movieId', 'title', 'genres'])
        ratings_df = pd.read_csv(ratings_path, usecols=['userId', 'movieId', 'rating', 'date'])
        
        # Конвертуємо date у datetime формат з обробкою помилок
        print("Converting date column to datetime...")
        ratings_df['date'] = pd.to_datetime(ratings_df['date'], errors='coerce')
        
        # Перевіряємо чи є проблемні дати
        problematic_dates = ratings_df['date'].isna().sum()
        if problematic_dates > 0:
            print(f"Warning: {problematic_dates} problematic dates found and set to NaT")
            # Показуємо перші кілька проблемних рядків
            problematic_rows = ratings_df[ratings_df['date'].isna()].head()
            print("First few problematic rows:")
            print(problematic_rows[['userId', 'movieId', 'rating', 'date']])
        
        # Видаляємо рядки з проблемними датами
        original_count = len(ratings_df)
        ratings_df.dropna(subset=['date'], inplace=True)
        print(f"Removed {original_count - len(ratings_df)} rows with invalid dates")
