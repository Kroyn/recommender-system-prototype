...
def load_data():
    ...
    try:
        ...
        movies = pd.read_csv('movies.csv', quotechar='"', escapechar='\\')
        ...
        try:
            ...
            print("Failed to parse movies.csv")
            return None, None, None

    # Додаємо рік фільму
    movies['year'] = movies['title'].str.extract(r'\((\d{4})\)').astype(float)
    
    # Конвертуємо дату у datetime та додаємо рік рейтингу
...
def prepare_yearly_stats(merged):
    ...
    yearly_stats = merged.groupby('rating_year').agg({
        'rating': ['mean', 'count']
    }).round(3)
